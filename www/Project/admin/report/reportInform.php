...
$id = $_GET['id'];
$stmt = $conn->prepare("select * from inform where InformID = $id");
$stmt->execute();
$result = $stmt->get_result();

$stmt = $conn->prepare("SELECT * FROM `inform` WHERE InformID = ?");
$stmt->bind_param("d", $id);
$stmt->execute();
$result1 = $stmt->get_result();
$row1 = mysqli_fetch_assoc($result1);

$stmt = $conn->prepare("select * from farmer where FM_id = ?");
$stmt->bind_param("d", $row1['FM_id']);
$stmt->execute();
$result2 = $stmt->get_result();
$row2 = mysqli_fetch_assoc($result2);
$datespilt = explode("-", $row1['date']);
if ($datespilt[1] == "01") {
    $datespilt[1] = "มกราคม";
...
$datethail = $datespilt[2] . "  " . $datespilt[1] . "  " . $datespilt[0];
...
<head>
    ...
    <title>ใบแจ้งน้ำหนักยางพารา</title>
</head>
                ...
                <div class="card">
                    <div class="card-header p-4">
                        <div class="float-right">
                            <h3 class="mb-0">ใบแจ้งน้ำหนักยางพารา</h3>
                            วันที่ : <?php echo $datethail ?>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="row mb-4">
                            ...
                            <div class="col-sm-6">
                                <div><img src="img/lanlogo.png" height="120" alt=""></div>
                                <h5 class="mb-3" class="text-dark mb-1">ถึง : <?php echo $row2['FM_name'] ?>
                                </h5>
                                <div><?php echo $row2['address'] ?></div>
                                <div>Email :<?php echo $row2['email'] ?></div>
                                <div>เบอร์โทร :<?php echo $row2['tel'] ?></div>
                            </div>
                        </div>
                            ...
                            <table class="table table-striped">
                                <thead>
                                    <tr>
                                        <th class="center">#</th>
                                        <th>ชื่อ - นามสกุล</th>
                                        <th class="right">น้ำหนักที่แจ้ง</th>
                                        <th class="center">จำนวนกระสอบ</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="right"><?php echo $row['weightItem'] ?></td>
                                        <td class="center"><?php echo $row['Num_item'] ?></td>
