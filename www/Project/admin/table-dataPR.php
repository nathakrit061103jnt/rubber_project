...
if (isset($_GET['id'])) {
    $userid = $_GET['id'];
    $stmt = $conn->prepare("DELETE FROM price_rubber WHERE PR_id=?");
    $stmt->bind_param("d", $userid);
    $stmt->execute();
    header("Location:table-dataPR.php");
}
    ...
    <div id="main-wrapper">
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">ข้อมูลราคายางพารา</h4>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td><button type="button" class="btn btn-primary" data-toggle="modal"
                                                        data-target="#exampleModal<?php echo $row['PR_id'] ?>">แก้ไข</button>
                                                    <a href="table-dataPR.php?id=<?php echo $row['PR_id'] ?>"
                                                        type="button"
                                                        onclick="return confirm('ต้องการลบข้อมูลใช่หรือไม่ ?')"
                                                        class="btn btn-danger">ลบ</a>
                                                    <a href="report/reportPR.php?date=<?php echo $row['date'] ?>"
                                                        type="button" class="btn btn-success" target="_blank">พิมพ์</a>
                                                </td>
                                            </tr>

                                            <div class=" modal fade" id="exampleModal<?php echo $row['PR_id'] ?>"
        ...
        ***********************************-->
        ...
        if (isset($_POST["SubmitUpdate"])) {
            ...
            $sqlUp = "UPDATE price_rubber p SET p.date = '{$_POST["date"]}', p.price = '{$_POST["price"]}'
            ...
            if (mysqli_query($conn, $sqlUp)) {
                ...
                "<script> 
                    Swal.fire({
                        ...
                        timer: 1500
                    }).then(()=> location = './table-dataPR.php')
                </script>";
            } else {
                echo
                "<script> 
