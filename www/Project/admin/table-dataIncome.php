...
$stmt = $conn->prepare("select * from income,employee where income.EM_id = employee.EM_id");
$stmt->execute();
$result = $stmt->get_result();
if (isset($_GET['id'])) {
    $userid = $_GET['id'];
    $stmt = $conn->prepare("DELETE FROM income WHERE incomeID=?");
    $stmt->bind_param("d", $userid);
    $stmt->execute();
    header("Location:table-dataIncome.php");
}
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">รายรับ - รายจ่าย</h4>
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered zero-configuration">
                                        <thead>
                                            <tr>
                                                <th>ลำดับ</th>
                                                <th>วันที่บันทึกรายรับ-รายจ่าย</th>
                                                <th>ส่วนคนขนยางพารา</th>
                                                <th>ส่วนเจ้าหน้าที่</th>
                                                <th>ส่วนเข้ากองทุน</th>
                                                <th>เจ้าหน้าที่ที่บันทึก</th>
                                                <th></th>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo $row['Balance'] * 0.4 ?></td>
                                                <td><?php echo $row['Balance'] * 0.3 ?></td>
                                                <td><?php echo $row['Balance'] * 0.3 ?></td>
                                                <td><?php echo $row['EM_name'] ?></td>
                                                <td><a href="table-dataIncome.php?id=<?php echo $row['IncomeID'] ?>"
                                                        type="button"
                                                        onclick="return confirm('ต้องการลบข้อมูลใช่หรือไม่ ?')"
                                                        class="btn btn-danger">ลบ</a>
                                                    <a href="report/reportIncome.php?date=<?php echo $row['date_income'] ?>"
